...
- **Authorization** (the fold, pure over the whole op-set): a normal op is counted **iff** its author device is not revoked **as of that op's HLC**. Computed from the `AuthorizeDevice`/`RevokeDevice` op-set, so it is order-independent and convergent. Ops by a device authored *before* its revocation stay valid; ops *after* are dropped. A key with no device cert resolves to **itself** as identity — so existing single-key data keeps working (graceful, non-breaking).
